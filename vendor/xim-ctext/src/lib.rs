...
pub fn compound_text_to_utf8(bytes: &[u8]) -> Result<String, DecodeError> {
    ...
    for chunk in split {
        ...
        match (iter.next(), iter.next()) {
            ...
            // UTF-8 End
            // ESC % @ only returns the decoder to the initial (ISO 8859-1)
            // state; the bytes after it in the same chunk are still text.
            // Discarding them loses characters whenever an XIM server emits
            // ASCII right after a non-ASCII run, e.g. the trailing "ng" of the
            // Vietnamese syllable "ti\u{1ebf}ng".
            (Some(0x25), Some(0x40)) => {
            ...
            // ASCII designated to G0 (ESC ( B), i.e. back to the initial state.
            // Same problem as above: only the escape itself is metadata, the
...
mod tests {
    ...
    #[test]
    fn utf8_end_then_ascii() {
        // As emitted by ibus-x11 for the Vietnamese syllable "ti\u{1ebf}ng ":
        // ASCII, an escaped UTF-8 run, a return to the initial state, then more
        // ASCII. The trailing "ng " must survive the round trip.
        const UTF8: &str = "ti\u{1ebf}ng ";
        const COMP: &[u8] = &[
            0x74, 0x69, 0x1b, 0x25, 0x47, 0xe1, 0xba, 0xbf, 0x1b, 0x25, 0x40, 0x6e, 0x67, 0x20,
        ];
        assert_eq!(crate::compound_text_to_utf8(COMP).unwrap(), UTF8);
    }

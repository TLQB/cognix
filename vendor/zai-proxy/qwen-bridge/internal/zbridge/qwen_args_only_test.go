...
func argOnlyCalls(t *testing.T) []map[string]interface{} {
	...
	text := "Tôi sẽ kiểm tra mức độ sẵn sàng release thực tế. <tool_call>\n" +
		`{"arguments":{"command":"echo '=== GIT STATUS ==='; git --no-optional-locks status --short","cd":"/home/tlqbao/Desktop/qwen-proxy","timeout_ms":120000,"head_lines":60}` +
		"\n</arguments>\n"
...
func TestArgOnly_MirrorCloser_RecoversCall(t *testing.T) {
	...
	if cd, _ := m["cd"].(string); cd != "/home/tlqbao/Desktop/qwen-proxy" {
...
func TestArgOnly_StreamPath_RecoversCall(t *testing.T) {
	...
	text := "Tôi sẽ kiểm tra. <tool_call>\n" +
		`{"arguments":{"command":"go test ./... -count=1","cd":"/home/tlqbao/Desktop/qwen-proxy","timeout_ms":120000}` +
		"\n</arguments>\nHoàn tất kiểm tra."

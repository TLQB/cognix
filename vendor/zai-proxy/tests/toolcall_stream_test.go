...
func mockToolCallUpstream(text string, every int) *httptest.Server {
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/v2/chat/completions" {
			// /api/models fetch (model resolution) and DELETE /api/v1/chats
			// (session GC) both hit the mock: answer models with the static
			// list, ignore the rest.
			if r.URL.Path == "/api/models" {
				w.Header().Set("Content-Type", "application/json")
				fmt.Fprint(w, `{"data":[{"id":"glm-4.7","name":"GLM-4.7","info":{"name":"GLM-4.7","meta":{"description":"mock","capabilities":{"enable_thinking":true}}}}]}`)
				return
			}
			w.WriteHeader(200)
			fmt.Fprint(w, "true")

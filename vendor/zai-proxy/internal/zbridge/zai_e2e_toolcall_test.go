...
func TestE2E_Streaming_KeyValueFormat(t *testing.T) {
	...
	block := "text <<<TOOL_CALL>>>\n" +
		`read_file path="zai-api/README.md" start_line="637" end_line="702"` +
		"\n</tool_call>\n"
	content, calls := feedSimulatedStream(t, block)
...
func TestE2E_StatusFromError_Coverage(t *testing.T) {
	cases := []struct {
		...
		{"Z.AI error 405: Method Not Allowed", 405},
		{"Z.AI error 401: unauthorized", 401},
		{"Z.AI error 403: forbidden", 403},
		{"Z.AI error 429: rate limited", 429},
		{"Z.AI error 400: bad request", 400},
		{"Z.AI error 502: bad gateway", 502},
		{"Z.AI error 503: service unavailable", 503},
		{"Z.AI error 504: gateway timeout", 504},
...
func TestSSEStream_ToolCallInterception(t *testing.T) {
	...
	toolCallJSON := `{"name":"read_file","arguments":{"path":"zai-proxy/internal/zbridge/zai.go","start_line":385,"end_line":595}}`
...
<<<END_TOOL_CALL>>>`
	...
	if !strings.Contains(args, "zai.go") {
		t.Errorf("arguments should contain 'zai.go', got %q", args)
	}
}

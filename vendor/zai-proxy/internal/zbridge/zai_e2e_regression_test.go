...
func TestRegression_DoubleAttempt_TagPseudoCallThenCanonical(t *testing.T) {
	...
	text := "Tôi sẽ xem xét cấu trúc code chính trong `internal/zbridge` để đánh giá kiến trúc chi tiết hơn.\n" +
		"<list_directory>\n" +
		`{"path":"zai-proxy/internal/zbridge"}` + "\n" +
		open + "\n" +
		`{"name":"list_directory","arguments":{"path":"zai-proxy/internal/zbridge"}}` + "\n" +
		closeM + "\n"
...
func TestRegression_DoubleAttempt_XMLStyleAtStreamEnd(t *testing.T) {
	...
	text := "Tôi sẽ xem xét code xử lý tool call trong `agent.go` và các file liên quan để đánh giá rủi ro.\n" +
		"<grep>" + `{"regex":"tool_call|tool_use|function_call","include_pattern":"zai-proxy/internal/zbridge/agent.go","offset":0}` + "</grep>\n" +
		"<Tool_Call>" + `{"name": "grep", "arguments": {"include_pattern": "zai-proxy/internal/zbridge/agent.go", "offset": 0, "regex": "tool_call|tool_use|function_call"}}` + "</Tool_Call>\n"
	...
	if args, ok := fn["arguments"].(string); ok {
		...
		if err := json.Unmarshal([]byte(args), &m); err == nil {
			if m["include_pattern"] != "zai-proxy/internal/zbridge/agent.go" {
...
func TestRegression_ReasoningTailCompletes_DeltaContent(t *testing.T) {
	...
	// Build SSE with delta_content appends (the common GLM streaming shape),
	// then the answer as content after </details>.
	var b strings.Builder
	b.WriteString("data: {\"data\":{\"delta_content\":\"<details>" + thinking + "\"}}\n\n")
	b.WriteString("data: {\"data\":{\"delta_content\":\"</details>" + answer + "\"}}\n\n")

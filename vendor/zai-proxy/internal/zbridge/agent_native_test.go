...
// exchanges are DEFUSED into text. The wire tool_calls / role:"tool" format
// makes Z.AI render <glm_block> into the model context; the model then emits
// its new tool calls in that poisoned <glm_block> format, which the backend
// rejects with INTERNAL_ERROR at end-of-stream (live-probed 2026-09-10:
// pivot case 4/4 crash with glm_block emission vs 4/4 pass text-folded).
// These tests pin the transform shape without network access.

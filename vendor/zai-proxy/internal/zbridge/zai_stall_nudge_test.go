...
// to `prompt`, which sendToZAIStream feeds into signature_prompt — a field
// Z.AI signs but never shows the model. The model answered the identical
...
func TestStallRetryNudgeReachesUpstream(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/v1/chats":
			writeJSONUpstream(w, 200, map[string]interface{}{"data": "chat-1"})
			return
		case "/api/v1/auths/":
			writeJSONUpstream(w, 200, map[string]interface{}{"data": map[string]string{"id": "u", "email": "e"}})
			return
		case "/api/models":
			writeJSONUpstream(w, 200, map[string]interface{}{"data": []map[string]interface{}{
				{"id": "glm-4.7", "name": "GLM-4.7", "info": map[string]interface{}{
					"name": "GLM-4.7", "meta": map[string]interface{}{
						"description": "mock", "capabilities": map[string]interface{}{"enable_thinking": true}}}},
			}})
			return
		case "/api/v2/chat/completions":
			...
			fmt.Fprint(w, "data: {\"data\":{\"phase\":\"other\",\"edit_index\":0,\"edit_content\":\"\"}}\n\n")
			if n == 1 {
				// Stall shape: announcement-only, no tool call.
				if sawNudge {
					t.Errorf("first attempt must not carry the nudge")
				}
				fmt.Fprint(w, "data: {\"data\":{\"edit_index\":0,\"edit_content\":\"I'll check the file now.\"}}\n\n")
			} else {
				...
				if !sawNudge {
					...
					nudged.Store(true)
					fmt.Fprint(w, "data: {\"data\":{\"edit_index\":0,\"edit_content\":\"<<<TOOL_CALL>>>{\\\"name\\\":\\\"read_file\\\",\\\"arguments\\\":{\\\"path\\\":\\\"a.go\\\"}}<<<END_TOOL_CALL>>>\"}}\n\n")
				}
			}
			fmt.Fprint(w, "data: {\"data\":{\"phase\":\"done\",\"done\":true}}\n\n")
			return
		default:

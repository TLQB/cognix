...
// a synthetic Z.AI event body and collects every ZAIResult in order.
func runSSEParser(t *testing.T, body string) []ZAIResult {
	t.Helper()
	ch := make(chan ZAIResult, 256)
...
// sseToolCallStream drives the full pipeline: upstream Z.AI SSE bytes →
...
func sseToolCallStream(t *testing.T, upstreamText string, chunkEvery int) (
	...
	for i := 0; i < len(upstreamText); i += chunkEvery {
		...
		payload, _ := json.Marshal(map[string]interface{}{"delta_content": upstreamText[i:end]})
		fmt.Fprintf(&body, "data: {\"data\":%s}\n\n", string(payload))
	}
	body.WriteString("data: {\"data\":{\"phase\":\"done\"}}\n\n")

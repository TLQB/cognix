...
func attachImageParts(transformed json.RawMessage, imageParts []map[string]interface{}) ([]byte, error) {
	...
	// Shims that preserve typed content arrays already carry the image
	// parts (post-rewrite file ids); only append parts whose url is not
	// present yet so a preserved array does not get duplicates.
	existing := make(map[string]bool, len(newParts))
	...
	for _, part := range imageParts {
		if u := extractImageURL(part); u != "" && existing[u] {
			continue
		}

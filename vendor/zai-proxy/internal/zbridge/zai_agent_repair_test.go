...
func TestAgentGLM52KeyValueWithXMLClose(t *testing.T) {
	var in AgentStreamInterceptor
	block := `Tôi sẽ tiếp tục phân tích cấu trúc dự án.<<<TOOL_CALL>>>read_file path="zai-api/README.md" start_line="637" end_line="702"
</tool_call>`
	var allCalls []map[string]interface{}
	for i := 0; i < len(block); i += 7 {
		end := i + 7
	...
	argsStr, _ := fn["arguments"].(string)
	if !strings.Contains(argsStr, "zai-api/README.md") {
		t.Errorf("arguments lost: %q", argsStr)
	}
}
...
func TestAgentTruncatedBracesRepair(t *testing.T) {
	...
	bodies := []string{
		`{"name":"terminal","arguments":{"command":"cat go.mod; echo '---BUILD---'; go build ./... 2>&1; echo '---VET---'; go vet ./... 2>&1","cd":"zai-proxy","timeout_ms":120000}`,
...
func TestAgentTruncatedBracesStreaming(t *testing.T) {
	var in AgentStreamInterceptor
	block := "Mình sẽ kiểm tra dependencies và chạy build/test.<<<TOOL_CALL>>>{\"name\":\"terminal\",\"arguments\":{\"command\":\"go vet ./...\",\"cd\":\"zai-proxy\",\"timeout_ms\":120000}" + "\n<<<END_TOOL_CALL>>>"
	var allCalls []map[string]interface{}
	var content string
	for i := 0; i < len(block); i += 7 {
		end := i + 7
...
func TestParseXMLTagToolCalls_NoNameInJSON(t *testing.T) {
	...
	text := "<terminal>\n" +
		`{"arguments":{"cd":"/home/tlqbao/Desktop/zai-proxy","command":"echo '=== CRATE COUNT ==='; ls crates | wc -l","timeout_ms":30000}}` +
		"\n" + closeM
...
func TestParseXMLTagToolCalls_TruncatedJSON(t *testing.T) {
	...
	jsonBody := `{"arguments":{"cd":"/home/tlqbao/Desktop/zai-proxy","command":"du -sh ."}}`[:len(`{"arguments":{"cd":"/home/tlqbao/Desktop/zai-proxy","command":"du -sh ."}}`)-1]
	text := "<terminal>\n" + jsonBody + "\n" + closeM
...
func TestParseBareEndToolCalls_MissingOpenMarker(t *testing.T) {
	closeM := "<<<" + "END_TOOL_CALL" + ">>>"
	text := "I'll check the README first.read_file" + `{"path":"zai-proxy/README.md","start_line":148,"end_line":260}` + "\n" + closeM

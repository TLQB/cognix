...
// 2026-09-10): Z.AI's /api/v2/chat/completions accepts a NATIVE
...
//     (Z.AI has no system role; the first user message is the convention
//     the modern shim already uses)
//
// BUT tool exchanges must NOT be sent in the OpenAI wire format
// (assistant.tool_calls / role:"tool"):
//
//   - Z.AI renders wire tool_calls into its own <glm_block> framing inside
//     the model context;
//   - the model then IMITATES that framing for NEW tool calls
//     (live-probed 2026-09-10, pivot case: <glm_block tool_call_name=...>
//     emitted, backend answered INTERNAL_ERROR at end-of-stream, 4/4 runs;
//     the text-folded control passed 4/4 with clean <<<TOOL_CALL>>> output).
//
...
func transformMessagesForAgentNative(rawMessages, toolsRaw json.RawMessage) ([]byte, error) {
	...
	//    Z.AI has no system role; it also matches what its own web client
	...
	//    are defused into text (see the header comment: wire tool_calls make
	//    Z.AI inject <glm_block> into the model context, and the model's own
	//    new calls then arrive in that poisoned format, which the backend
	//    rejects with INTERNAL_ERROR).
	for _, m := range msgs {
		switch m.Role {
		case "system", "developer":
			// Z.AI has no system role; fold into the contract message's

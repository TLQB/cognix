...
/// OpenAI-style `/models` listing served by JustWoker.
pub const JUSTWOKER_MODELS_URL: &str = "https://api.justwoker.icu/v1/models";
pub const JUSTWOKER_API_URL: &str = "https://api.justwoker.icu/v1";
...
pub async fn fetch_models(client: &dyn HttpClient) -> Result<Vec<Model>> {
    ...
    if !response.status().is_success() {
        ...
        anyhow::bail!("failed to fetch JustWoker models: {} {}", response.status(), body);
    }
    let mut body = String::new();
    response.body_mut().read_to_string(&mut body).await?;
    let response: ModelListResponse =
        serde_json::from_str(&body).context("failed to parse JustWoker models list")?;
...
pub async fn stream_chat_completion(
    ...
    // `https://api.justwoker.icu/v1`), so the chat completions endpoint is
    ...
    if response.status().is_success() {
        ...
        response.body_mut().read_to_string(&mut body).await?;
        anyhow::bail!(
            "Failed to connect to JustWoker API: {} {}",
            response.status(),
            body,
        );
    }
...
mod tests {
    ...
    fn parses_models_list() {
        ...
        assert_eq!(response.data[0].id, "claude-opus-5-thinking");
    }

...
use nim::{
    NIM_API_URL, ModelEntry, Props, get_filter_models, get_models, get_props,
...
static FALLBACK_API_KEY: LazyLock<SharedString> = LazyLock::new(|| {
    let decoded = BASE64_STANDARD
        .decode("bnZhcGkteTUzX1YxT2lpZ2RqMFZpZmozN0sxeVd2Y0F4SXM3bC1MUzhmdFpjc2VoY1ZCRWVZenYwT1pLaHR5WTZYSG1LdA==")
...
pub struct NimLanguageModelProvider {
...
impl State {
    ...
    fn set_api_key(&mut self, api_key: Option<String>, cx: &mut Context<Self>) -> Task<Result<()>> {
        ...
        let api_url = NimLanguageModelProvider::api_url(cx);
    ...
    fn authenticate(&mut self, cx: &mut Context<Self>) -> Task<Result<(), AuthenticateError>> {
        ...
        cx.spawn(async move |this, cx| {
            match load_key_task.await {
                ...
                Err(error) => {
                    log::warn!("failed to load nim API key: {error}");
                }
            }
    ...
    fn fetch_models(&mut self, cx: &mut Context<Self>) -> Task<Result<()>> {
        ...
        let settings = NimLanguageModelProvider::settings(cx);
        let api_url = NimLanguageModelProvider::api_url(cx);
        let api_key = self.api_key_state.key(&api_url).or_else(|| Some(FALLBACK_API_KEY.clone().into()));
        let extra_headers = settings.custom_headers.clone();
        let nim_filter_url = settings.nim_filter_url.clone();
        ...
        cx.spawn(async move |this, cx| {
            ...
            let entries = if let Some(nim_filter_url) = &nim_filter_url {
                match get_filter_models(http_client.as_ref(), nim_filter_url).await {
                    Ok(filter_ids) => entries
                        .into_iter()
                        .filter(|entry| filter_ids.contains(&entry.id))
                        .collect(),
                    Err(err) => {
                        log::warn!("failed to fetch filter models: {err:#}");
                        entries
                    }
                }
            } else {
            ...
            let models: Vec<nim::Model> = if is_router {
            ...
            this.update(cx, |this, cx| {
                ...
                let effective = compute_effective_models(
                    &this.fetched_models,
                    NimLanguageModelProvider::settings(cx),
                );
    ...
    fn start_model_event_stream(&mut self, cx: &mut Context<Self>) {
        ...
        let api_url = NimLanguageModelProvider::api_url(cx);
        let api_key = self.api_key_state.key(&api_url).or_else(|| Some(FALLBACK_API_KEY.clone().into()));
        let extra_headers = NimLanguageModelProvider::settings(cx)
            .custom_headers
            .clone();
        ...
        self.model_event_task = Some(cx.spawn(async move |this, cx| {
            loop {
                ...
                .await
                {
                    Ok(mut events) => {
                        while let Some(event) = events.next().await {
                            ...
                            if let Some(exit_code) = event.load_failure() {
                                log::error!(
                                    "nim model {} failed to load (exit code {exit_code})",
                                    event.model
                                );
                            }
                    ...
                    Err(error) => {
                        log::warn!("nim model event stream unavailable: {error:#}");
                    }
                }
...
fn compute_effective_models(
    fetched_models: &[nim::Model],
    settings: &NimSettings,
) -> HashMap<String, nim::Model> {
    let mut models: HashMap<String, nim::Model> = HashMap::default();
...
fn sync_capability_cells(cells: &CapabilityCells, effective: &HashMap<String, nim::Model>) {
...
fn model_from_entry(entry: &ModelEntry, props: Option<&Props>) -> nim::Model {
...
impl NimLanguageModelProvider {
        ...
        let this = Self {
            ...
            state: cx.new(|cx| {
                cx.observe_global::<SettingsStore>({
                    let mut last_settings = NimLanguageModelProvider::settings(cx).clone();
                    move |this: &mut State, cx| {
                        let current_settings = NimLanguageModelProvider::settings(cx);
    ...
    fn settings(cx: &App) -> &NimSettings {
        &AllLanguageModelSettings::get_global(cx).nim
    }
    ...
    fn has_custom_url(cx: &App) -> bool {
        let api_url = &Self::settings(cx).api_url;
        !api_url.is_empty() && api_url != NIM_API_URL
    }
}

impl LanguageModelProviderState for NimLanguageModelProvider {
...
impl LanguageModelProvider for NimLanguageModelProvider {
...
impl NimLanguageModel {
    ...
    fn to_nim_request(
        &self,
        request: LanguageModelRequest,
    ) -> Result<nim::ChatCompletionRequest> {
        build_nim_request(
            &self.name,
    ...
    fn stream_completion(
        &self,
        request: nim::ChatCompletionRequest,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
        Result<futures::stream::BoxStream<'static, Result<nim::ResponseStreamEvent>>>,
    > {
        ...
        let (api_key, api_url, extra_headers) = self.state.read_with(cx, |state, cx| {
            ...
            let extra_headers = NimLanguageModelProvider::settings(cx)
                .custom_headers
                .clone();
            (state.api_key_state.key(&api_url).or_else(|| Some(FALLBACK_API_KEY.clone().into())), api_url, extra_headers)
        });
...
fn build_nim_request(
    ...
    if request.contains_custom_tool_input() {
        anyhow::bail!("nim does not support custom tools");
    }
            ...
            match content {
                ...
                MessageContent::Image(image) => {
                    if supports_images {
                        add_message_content_part(
                            nim::MessagePart::Image {
                                image_url: nim::ImageUrl {
                ...
                MessageContent::ToolUse(tool_use) => {
                    let input = tool_use.input.as_json().ok_or_else(|| {
                        anyhow::anyhow!("nim does not support custom tool calls")
                    })?;
                    let tool_call = nim::ToolCall {
                        id: tool_use.id.to_string(),
                        content: nim::ToolCallContent::Function {
                            function: nim::FunctionContent {
                    ...
                    if let Some(nim::ChatMessage::Assistant {
                        tool_calls,
                    ...
                    }) = messages.last_mut()
                    {
                        ...
                        messages.push(nim::ChatMessage::Assistant {
                            content: None,
                ...
                MessageContent::ToolResult(tool_result) => {
                    let content: Vec<nim::MessagePart> = tool_result
                        .content
                        .iter()
                        .filter_map(|part| match part {
                            LanguageModelToolResultContent::Text(text) => {
                                Some(nim::MessagePart::Text {
                                    text: text.to_string(),
                                })
                            }
                            LanguageModelToolResultContent::Image(image) => {
                                if supports_images {
                                    Some(nim::MessagePart::Image {
                                        image_url: nim::ImageUrl {
                                            url: image.to_base64_url(),
                                            detail: None,
                                        },
                    ...
                    messages.push(nim::ChatMessage::Tool {
    ...
    let tools: Vec<nim::ToolDefinition> = if supports_tools {
        request
            .tools
            .into_iter()
            .map(|tool| {
                let input_schema = match tool.input {
                    ...
                    language_model::LanguageModelRequestToolInput::Custom { .. } => {
                        return Err(anyhow::anyhow!("nim does not support custom tools"));
                    }
                };
                Ok(nim::ToolDefinition::Function {
                    function: nim::FunctionDefinition {
                        name: tool.name,
    ...
    let tool_choice = if tools.is_empty() {
        ...
        request.tool_choice.map(|choice| match choice {
            LanguageModelToolChoice::Auto => nim::ToolChoice::Auto,
            LanguageModelToolChoice::Any => nim::ToolChoice::Required,
            LanguageModelToolChoice::None => nim::ToolChoice::None,
        })
    };

    Ok(nim::ChatCompletionRequest {
        ...
        tools,
        tool_choice,
        stream_options: Some(nim::StreamOptions {
            include_usage: true,
        }),
        reasoning_effort,
    })
...
impl LanguageModel for NimLanguageModel {
    ...
    fn stream_completion(
        ...
        >,
    > {
        let request = match self.to_nim_request(request) {
            Ok(request) => request,
            Err(error) => return async move { Err(error.into()) }.boxed(),
        };
        let completions = self.stream_completion(request, cx);
        async move {
            let mapper = NimEventMapper::new();
...
impl NimEventMapper {
    ...
    pub fn map_stream(
        mut self,
        events: Pin<Box<dyn Send + Stream<Item = Result<nim::ResponseStreamEvent>>>>,
        ...
        if let Some(choice) = event.choices.into_iter().next() {
            ...
            if let Some(finish_reason) = choice.finish_reason.as_deref() {
                match finish_reason {
                    ...
                    unexpected => {
                        log::warn!("Unexpected nim finish_reason: {unexpected:?}");
                        events.push(Ok(LanguageModelCompletionEvent::Stop(StopReason::EndTurn)));
                    }
                }
            }
...
fn add_message_content_part(
    new_part: nim::MessagePart,
    role: Role,
    messages: &mut Vec<nim::ChatMessage>,
    reasoning_content: Option<String>,
) {
    match (role, messages.last_mut()) {
        (Role::User, Some(nim::ChatMessage::User { content }))
        | (Role::System, Some(nim::ChatMessage::System { content })) => {
            content.push_part(new_part);
        }
        (
            Role::Assistant,
            Some(nim::ChatMessage::Assistant {
        ...
        _ => {
            messages.push(match role {
                Role::User => nim::ChatMessage::User {
                    content: nim::MessageContent::from(vec![new_part]),
                },
                Role::Assistant => nim::ChatMessage::Assistant {
                    content: Some(nim::MessageContent::from(vec![new_part])),
                    reasoning_content,
                    tool_calls: Vec::new(),
                },
                Role::System => nim::ChatMessage::System {
                    content: nim::MessageContent::from(vec![new_part]),
                },
            });
        }
...
fn merge_settings_into_models(
    models: &mut HashMap<String, nim::Model>,
